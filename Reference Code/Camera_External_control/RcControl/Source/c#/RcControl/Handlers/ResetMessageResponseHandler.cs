...
{
    public class ResetMessageResponseHandler : SysexMessageHandler<ResetMessageResponse>
    {
        #region Variables
        private const byte CommandByte = RcCommands.RESET;

        protected new const string BaseExceptionMessage =
            "Error with the incoming byte. This is not a valid ResetMessageResponse. ";
        ...
        #region Ctor / Dtor
        public ResetMessageResponseHandler(IMessageBroker messageBroker)
            : base(messageBroker)
        { }
        #endregion
        ...
        protected override bool HandleByte(byte messageByte)
        {
            switch (currentHandlerState)
            {
                ...
                case HandlerState.StartSysex:
                    if (messageByte != RcCommands.RESET)
                    {
                        currentHandlerState = HandlerState.StartEnd;
                        throw new MessageHandlerException(BaseExceptionMessage +
                            String.Format("Command byte not {0:X}", RcCommands.RESET));
                    }
