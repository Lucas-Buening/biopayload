...
{
    public class SetChannelValueMessageCreator : BaseMessageCreator<SetChannelValueMessage>
    {
        public override byte[] CreateMessage(SetChannelValueMessage message)
        {
            if (message == null)
            ...
            byte[] buffer = new byte[] 
            {
                ...
                RcCommands.SET_CHANNEL_VALUE,
                (byte)(message.Channel & 0x7f),
                (byte)(message.Value & 0x7f),
                (byte)((message.Value >> 7) & 0x7f),
